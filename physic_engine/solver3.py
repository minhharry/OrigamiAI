...
class OrigamiObjectMatrix:
    ...
    def step(self):
        ...
        lensq_n1 = torch.clamp(lensq_n1, min=EPS)
        lensq_n2 = torch.clamp(lensq_n2, min=EPS)
        
        len_p3p4 = torch.linalg.norm(vec_p3p4, dim=2, keepdim=True)
        len_p3p4 = torch.clamp(len_p3p4, min=EPS)
        ...
        if self.use_projection:
            ...
            lensq_p3p4 = torch.clamp(lensq_p3p4, min=EPS)
...
def origami_step_jax(state, params):
    ...
    lensq_n1 = jnp.maximum(lensq_n1, EPS)
    lensq_n2 = jnp.maximum(lensq_n2, EPS)
    
    len_p3p4 = jnp.linalg.norm(vec_p3p4, axis=1, keepdims=True)
    len_p3p4 = jnp.maximum(len_p3p4, EPS)
    ...
    lensq_p3p4 = jnp.maximum(lensq_p3p4, EPS)
...
if __name__ == "__main__":
    ...
    with open(r"test_fold\M.json", "r") as f:
        data = json.load(f)

    points, edges, faces, target_theta = [], [], [], []
    for key, node in data["nodes"].items():
        points.append([node[0], node[1], 0.0])

    for edge in data["edges"]:
        edges.append([edge["u"], edge["v"]])
        if edge["attributes"][0] is not None and edge["attributes"][1] is not None and edge["attributes"][2] is not None and edge["attributes"][3] is not None:
            faces.append([edge["attributes"][0], edge["attributes"][1], edge["attributes"][2], edge["attributes"][3]])
            target_theta.append([edge["value"]])

    points = torch.tensor(points)
    edges = torch.tensor(edges)
    faces = torch.tensor(faces)
    target_theta = torch.tensor(target_theta)
